...
<body>
    @php
        ...
        $bln = $id_bln;
    @endphp
    <h2 class="text-center">PROSES PENGADAAN BARANG DAN JASA PAKET NON STRATEGIS (>RP. 50 JUTA S/D Rp. 200 JUTA)
        <br>{{ $unit->nm_unit }}
    </h2>
    ...
    <table border="1" width="100%" cellpadding="5">
        ...
        <tr>
            ...
            <th width="3%" class="text-center">{{ $ppbj->jml_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->jml_pg_50, 0, ',', '.') }}</th>
            <th width="3%" class="text-center">{{ $ppbj->pl_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->pl_rp_50, 0, ',', '.') }}</th>
            <th width="3%" class="text-center">{{ $ppbj->h_pl_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->h_pl_rp_50, 0, ',', '.') }}</th>
            <th width="3%" class="text-center">{{ $ppbj->kontrak_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->kontrak_rp_50, 0, ',', '.') }}</th>
            <th width="3%" class="text-center">{{ $ppbj->st_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->st_rp_50, 0, ',', '.') }}</th>
            <th width="3%" class="text-center">{{ $ppbj->bp_pkt_50 }}</th>
            <th class="text-right">{{ number_format($ppbj->bp_rp_50, 0, ',', '.') }}</th>
